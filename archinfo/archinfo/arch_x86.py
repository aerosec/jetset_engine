...
class ArchX86(Arch):
    ...
    def stack_push(self, state, thing):
        """
        Push 'thing' to the stack, writing the thing to memory and adjusting the stack pointer.
        """
        # increment sp
        print(" >>> This is the wrong stack push :( <<<")
        sp = state.regs.sp + state.arch.stack_change
        state.regs.sp = sp
        #sp = x86g_use_seg_selector(self,self.gdt,self.ldt,self.regs.ss,sp)
        return state.memory.store(sp, thing, endness=state.arch.memory_endness)

    def stack_pop(self, state):
        """
        Pops from the stack and returns the popped thing. The length will be the architecture word size.
        """
        print(" >>> This is the wrong stack pop :( <<<")
        sp = state.regs.sp
        state.regs.sp = sp - state.arch.stack_change
        #print(" -- This is a secret x86 pop! --")
        return state.memory.load(sp, state.arch.bytes, endness=state.arch.memory_endness)

    def stack_read(self, state, offset, length, bp=False):
        """
        ...
        """
        #print("This is a special x86 stack read!")
        print(" >>> This is the wrong stack read :( <<<")
        sp = state.regs.bp if bp else state.regs.sp
        #    sp = x86g_use_seg_selector(self,self.gdt,self.ldt,self.regs.ss,sp)
        return state.memory.load(sp+offset, length, endness=state.arch.memory_endness)
    ...
    register_list = [
        Register(name='eax', size=4, subregisters=[('ax', 0, 2),
                                                   ('al', 0, 1),
                                                   ('ah', 1, 1)],
