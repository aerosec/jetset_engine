...
static void hook_bl_arm(uc_engine *uc, uint32_t target, void *user_data);
static void hook_unlink_arm(uc_engine *uc, uint64_t address, int32_t size, void *user_data);
static void hook_call_m68k(uc_engine *uc, uint32_t target, uint32_t ret_addr, void *user_data);
static void hook_rts_m68k(uc_engine *uc, uint32_t target, void *user_data);
...
private:
	...
	std::vector<mem_access_t> mmio_writes;
...
public:
	...
	std::vector<std::pair<uint64_t, uint64_t>> mmio_regions;

	std::vector<std::tuple<uint64_t, uint64_t, uint64_t, uint64_t>> callstack;
...
#define ADD_ARCH_HOOK(...) {                       \
	arch_hook = new uc_hook;                       \
	err = uc_hook_add(uc, arch_hook, __VA_ARGS__); \
	arch_hooks.push_back(arch_hook);               \
}
		if (arch == UC_ARCH_ARM) {
			// ADD_ARCH_HOOK(UC_HOOK_INSN, (void *)hook_bl_arm, this, 1, 0, UC_ARM_INS_BL);
			// ADD_ARCH_HOOK(UC_HOOK_BLOCK, (void *)hook_unlink_arm, this, 1, 0);
		}
		if (arch == UC_ARCH_M68K) {
			// ADD_ARCH_HOOK(UC_HOOK_INSN, (void *)hook_call_m68k, this, 1, 0, UC_M68K_INS_CALL);
			// ADD_ARCH_HOOK(UC_HOOK_INSN, (void *)hook_rts_m68k, this, 1, 0, UC_M68K_INS_RTS);
		}
#undef ADD_ARCH_HOOK
	...
	void unhook() {
		...
		for (auto it = arch_hooks.begin(); it != arch_hooks.end(); it++) {
			err = uc_hook_del(uc, **it);
			delete *it;
		}
		arch_hooks.clear();
	}
	...
	bool is_mmio_addr(uint64_t address) {
		for (auto it = mmio_regions.begin(); it != mmio_regions.end(); it++) {
	 ...
	 */
	bool log_write(uint64_t address, int size, int64_t value, int clean) {
		mem_access_t record;
	...
	void commit() {
		...
		for (auto it = mem_writes.begin(); it != mem_writes.end(); it++) {
			bool is_mmio = is_mmio_addr(it->address);
			taint_t taint_type = is_mmio ? TAINT_SYMBOLIC : TAINT_DIRTY;

			if (it->clean == -1) {
				taint_t *bitmap = page_lookup(it->address);
				memset(&bitmap[it->address & 0xFFFULL], taint_type, sizeof(taint_t) * it->size);
				it->clean = (1 << it->size) - 1;
				//LOG_D("commit: lazy initialize mem_write [%#lx, %#lx]", it->address, it->address + it->size);
			}
			if (is_mmio)
				mmio_writes.push_back(*it);
		}
	...
	void page_activate(uint64_t address, uint8_t *taint = NULL, uint64_t taint_offset = 0) {
		...
		taint_t taint_type = is_mmio_addr(address) ? TAINT_SYMBOLIC : TAINT_DIRTY;
		...
		for (auto a = mem_writes.begin(); a != mem_writes.end(); a++)
			if (a->clean == -1 && (a->address & ~0xFFFULL) == address) {
				...
				memset(&bitmap[a->address & 0xFFFULL], taint_type, sizeof(taint_t) * a->size);
	...
	mem_update_t *sync() {
		...
		for (auto it = mmio_writes.rbegin(); it != mmio_writes.rend(); it++) {
			// fprintf(stderr, ">>>>>>>>>>>>> Memory is being WRITE at 0x%x, data size = %d\n", it->address, it->size);
			mem_update_t *range = new mem_update_t;
			range->address = it->address;
			range->length = it->size;
			range->has_value = true;
			range->value = it->new_value;
			range->next = head;
			head = range;
		}
	...
	void handle_write(uint64_t address, int size, int64_t value)
	{
		taint_t *bitmap = page_lookup(address);
		int start = address & 0xFFF;
		int end = (address + size - 1) & 0xFFF;
		...
		if (end >= start) {
			if (bitmap) {
				clean = 0;
				for (int i = start; i <= end; i++) {
					if (bitmap[i] != taint_type) {
						clean |= (1 << i); // this bit should not be marked as taint if we undo this action
						bitmap[i] = taint_type; // this will automatically remove TAINT_SYMBOLIC flag
					}
				}
			} else {
				clean = -1;
			}
			log_write(address, size, value, clean);
		} else {
			// Handling MMIO while the memory is looping around is too much
			stop(STOP_ERROR);
		}
	}
...
static void hook_mem_read(uc_engine *uc, uc_mem_type type, uint64_t address, int size, int64_t value, void *user_data) {
	...
	} else if (state->is_mmio_addr(address)) {
...
static void hook_mem_write(uc_engine *uc, uc_mem_type type, uint64_t address, int size, int64_t value, void *user_data) {
	...
	state->handle_write(address, size, value);
}
...
static void hook_intr(uc_engine *uc, uint32_t intno, void *user_data) {
	...
	if (state->arch == UC_ARCH_X86 && intno == 0x80) {
		...
		if (sysno == state->transmit_sysno) {
			...
			if (fd == 2) {
				...
				if (tx_bytes != 0) state->handle_write(tx_bytes, 4, count);
	...
	} else if (state->arch == UC_ARCH_M68K) {
		// Unicorn will consider the interruot handled by hook and do not stop execution
		state->stop(STOP_SYSCALL);
	}
}
...
static void hook_bl_arm(uc_engine *uc, uint32_t target, void *user_data) {
	...
	if (lr < 0x1000) // this is reserved memory, we should not be returning to there
		return;

	state->callstack.push_back(std::make_tuple(pc, target, sp, lr));
}
...
static void hook_unlink_arm(uc_engine *uc, uint64_t address, int32_t size, void *user_data) {
	...
	auto frame = state->callstack.begin();
	for (; frame != state->callstack.end(); frame++) {
		uint64_t call_site_addr, func_addr, stack_ptr, ret_addr;
		std::tie(call_site_addr, func_addr, stack_ptr, ret_addr) = *frame;

		if (ret_addr && ret_addr == address && stack_ptr == sp)
			goto found;
	}
...
found:
	state->callstack.erase(frame, state->callstack.end());
}
...
static void hook_call_m68k(uc_engine *uc, uint32_t target, uint32_t ret_addr, void *user_data) {
	...
	state->callstack.push_back(std::make_tuple(pc, target, sp, ret_addr));
}

static void hook_rts_m68k(uc_engine *uc, uint32_t target, void *user_data) {
	State *state = (State *)user_data;

	assert(!state->callstack.empty());
...
// MMIO
extern "C"
void simunicorn_add_mmio_region(State *state, uint64_t addr_lower, uint64_t addr_upper) {
	state->mmio_regions.push_back(std::make_pair(addr_lower, addr_upper));
}

extern "C"
void simunicorn_clear_mmio_regions(State *state) {
	state->mmio_regions.clear();
}

// Callstack
extern "C"
void simunicorn_callstack_set(State *state, PyObject *obj) {
	...
	while (item = PyIter_Next(iterator)) {
		uint64_t call_site_addr, func_addr, stack_ptr, ret_addr;
		if (!PyArg_ParseTuple(item, "nnnn;Callstack items must be tuple of 4 elements", &call_site_addr, &func_addr, &stack_ptr, &ret_addr)) {
			Py_DECREF(item);
			break;
		}

		state->callstack.insert(state->callstack.begin(), std::make_tuple(call_site_addr, func_addr, stack_ptr, ret_addr));
...
extern "C"
PyObject *simunicorn_callstack_get(State *state) {
	...
	for (auto it = state->callstack.rbegin(); it != state->callstack.rend(); it++) {
		uint64_t call_site_addr, func_addr, stack_ptr, ret_addr;
		std::tie(call_site_addr, func_addr, stack_ptr, ret_addr) = *it;

		PyObject *item = Py_BuildValue("(nnnn)", call_site_addr, func_addr, stack_ptr, ret_addr);
		if (!item)
			goto err;
