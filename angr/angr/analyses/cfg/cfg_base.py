...
class CFGBase(Analysis):
    ...
    def _post_analysis(self,incremental=False):
    ...
    def _check_for_arm_tailcalls(self, func):
        for callsite in func.get_call_sites(): 
            block = func._get_block(callsite)  
            if block.capstone.insns[-1].mnemonic  == 'b': 
                target = func.get_call_target(callsite) 
                target_func = self.functions[target] 
    ...
    def _analyze_function_features(self):
        ...
        for func in all_functions:  # type: angr.knowledge.Function
            ...
            # mark as returning if we follow a tail call that returns
            if self.project.arch.name == 'ARMEL':
                if self._check_for_arm_tailcalls(func):
                    func.returning = True 
                    continue
    ...
    def make_functions(self,incremental=False):
        """
        ...
        if incremental:
            for func in tmp_functions.values():
                for baddr in func.block_addrs:
                    blockaddr_to_function[baddr] = func
        ...
        for i, fn in enumerate(sorted(function_nodes, key=lambda n: n.addr)):
            #if incremental and fn.addr in tmp_functions:
            #    func = tmp_functions[fn.addr]
            #    if func.normalized:
            #        continue
        ...
        for func_addr in tmp_functions:
            #if incremental and tmp_functions[func_addr].normalized:
            #    continue
                ...
                for src, dst, data in all_out_edges:
                    ...
                    if not (jumpkind == 'Ijk_Call' or jumpkind.startswith('Ijk_Sys')):
                        ...
                        if dst not in stack and dst not in traversed:
                            stack.add(dst)
    # HACKY: this is a very simple way to detect tail calls in arm
    # tail calls mostly take the form of pop lr -> B xxxx
    # this is done in pyvex postprocessing
    # if you run into a problem like this, yell at Evan
    def _is_arm_tailcall(self, n):
        if self.project.arch.name == 'ARMEL' and n.block.capstone.insns[-1].mnemonic == 'b':
            return True
        return False          
    ...
    def _graph_traversal_handler(self, src, dst, data, blockaddr_to_function, known_functions, all_edges):
        ...
        if jumpkind == 'Ijk_Call' or jumpkind.startswith('Ijk_Sys'):
            ...
            if not all_edges or (dst_function.returning is False and not dst_function.name == b'UnresolvableTarget') or self._is_arm_tailcall(n):
                fakeret_node = None
            else:
            ...
            if dst_function.returning and not self._is_arm_tailcall(n):
