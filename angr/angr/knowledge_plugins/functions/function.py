...
class Function(object):
    ...
    def _is_arm_tailcall(self, n):
        try:
            block = self._project.factory.block(n.addr)
            if self._project.arch.name == 'ARMEL' and (block.size > 0) and block.capstone.insns[-1].mnemonic  == 'b':
                return True
        except:
            print("_is_arm_tailcall in knowledge_plugins/functions/functions.py failed")
        return False
        ...
        for src, dst, data in self.transition_graph.edges(data=True):
            if 'type' in data and data['type'] == 'call':
                ...
                if func_addr in self._function_manager:
                    ...
                    if function.returning is False or self._is_arm_tailcall(self.get_node(src.addr)):
                        ...
                        #if function.returning and self._is_arm_tailcall(the_node):
                        #    self.returning = True
