...
data['AMD64']['CondBitOffsets']['G_CC_SHIFT_IF'] = 9
...
data['AMD64']['CondBitMasks']['G_CC_MASK_IF'] = (1 << data['AMD64']['CondBitOffsets']['G_CC_SHIFT_IF'])
...
data['X86']['CondBitOffsets']['G_CC_SHIFT_IF'] = 9
...
data['X86']['CondBitMasks']['G_CC_MASK_IF'] = (1 << data['X86']['CondBitOffsets']['G_CC_SHIFT_IF'])
...
def pc_calculate_rdata_all_WRK(state, cc_op, cc_dep1_formal, cc_dep2_formal, cc_ndep_formal, platform=None):
    ...
    if cc_op == data[platform]['OpTypes']['G_CC_OP_COPY']:
        ...
        return cc_dep1_formal & (data[platform]['CondBitMasks']['G_CC_MASK_O'] | data[platform]['CondBitMasks']['G_CC_MASK_S'] | data[platform]['CondBitMasks']['G_CC_MASK_Z']
              | data[platform]['CondBitMasks']['G_CC_MASK_A'] | data[platform]['CondBitMasks']['G_CC_MASK_C'] | data[platform]['CondBitMasks']['G_CC_MASK_P'] | data[platform]['CondBitMasks']['G_CC_MASK_IF'])
...
def x86g_get_seg_limit(state,ldt,gdt,seg_selector):
    seg_selector &= 0x0000FFFF
    tiBit = (seg_selector >> 2) & 1
    #print("Get seg limit",seg_selector)
    ...
    elif state.solver.is_true(tiBit == 1):
        ldt_base = ldt[47:16]
        ldt_base_value = state.solver.eval_one(ldt_base)
        descriptor = state.memory.load(ldt_base_value + seg_selector * 8, 8, endness='Iend_LE')

    limit = get_segdescr_limit(state, descriptor)
    #print("Get Seg Limit {}".format(limit))
    return limit, ()
...
def x86g_use_seg_selector(state, ldt, gdt, seg_selector, virtual_addr):
    ...
    seg_selector &= 0x0000FFFF
    #print("Seg Selector: ",seg_selector," Virtual Addr: ",virtual_addr)
    # RPL=11 check
    ...
    if state.solver.is_true(tiBit == 0):
        ...
        seg_selector >>= 3 # bit 3 to 15 are the index in the table
        seg_selector = seg_selector.zero_extend(32)
        #print("Index into GDT: ",seg_selector)
        gdt_limit = gdt[15:0]
        ...
        gdt_base_value = state.solver.eval_one(gdt_base)
        descriptor = state.memory.load(gdt_base_value + seg_selector * 8, 8, endness='Iend_LE')
        #print("{}(Descriptor_Addr) = {}(gdt_base_value) + {} (seg_idx * 8) ".format((gdt_base_value + seg_selector * 8),hex(gdt_base_value),seg_selector))
        #print("Descriptor = ", descriptor)
        
    elif state.solver.is_true(tiBit == 1):
        # LDT access
        #print(" === LDT ACCESS! ==== ldt = ", ldt, " @ ", state.regs.eip)
        ldt_value = state.solver.eval_one(ldt)
        #print("seg_selector {}: {} (ldt_value) + {}(vaddr)".format(seg_selector * 8,ldt_value,virtual_addr))
        if ldt_value == 0:
            return ((seg_selector << 16) + virtual_addr).zero_extend(32), ()
        ...
        #    return bad("index out of range")
        #print("LDT limit = ", ldt_limit)

        ldt_base = ldt[47:16]
        #print("ldt_base = ", ldt_base)
        ldt_base_value = state.solver.eval_one(ldt_base)

        ldt_value = state.solver.eval_one(ldt_base)
        descriptor = state.memory.load(ldt_value + seg_selector * 8, 8, endness='Iend_LE')
        #print("seg_selector {}: {}(base) + {}(virt_addr) = {}(result)".format(seg_selector * 8,ldt_base,virtual_addr,r))
    else:
        l.error("Symbolic seg_selector = " + str(seg_selector << 3))
        return bad("tiBit could not be concretized, likely a symbolic segment selector")
    present = descriptor[47]
    if state.solver.is_true(present == 0):
        print("present bit set to 0 @", hex(state.addr))
        print("GDT = ", gdt, " Seg selector = ", seg_selector << 3, "virtual_addr", virtual_addr)
    ...
    if state.solver.is_true(virtual_addr >= limit):
        print("virtual_addr >= limit @ ", hex(state.addr))
        print("GDT = ", gdt, " Seg selector = ", seg_selector << 3, " virtual_addr", virtual_addr, " limit = ", limit )
        return bad("virtual_addr >= limit")

    r = (base + virtual_addr).zero_extend(32)
    l.debug("x86g_use_seg_selector: addr=%s", str(r))
    #if not state.solver.is_true(tiBit == 0):
    #print("seg_selector {}: {}(base) + {}(virt_addr) = {}(result) @ {}".format(seg_selector * 8,base,virtual_addr,r,state.regs.eip))
    if hasattr(state,'debug'):
        state.debug.trace.append( ('t',r))
    return r, ()
...
def armg_msr(state):
    print("MSR is happening")
    if hasattr(state,'coproc'):
        state.coproc.trace.append( ('msr', state.regs.pc) )
    return state.solver.BVV(0, 32), []

def armg_mrs(state):
    print("MRS is happening")
    if hasattr(state,'coproc'):
        state.coproc.trace.append( ('mrs', state.regs.pc) )
    return state.solver.BVV(0, 32), []
    

def armg_mcr(state):
    if hasattr(state,'coproc'):
        state.coproc.trace.append( ('mcr', state.regs.pc) )
    return state.solver.BVV(0, 32), []

def armg_mrc(state):
    v = state.solver.Unconstrained('COPROC', 32, key=('hardware', 'coproc'))
    if hasattr(state,'coproc'):
        state.coproc.trace.append( ('mrc', state.regs.pc, v) )
    return v, []

def armg_mrc_device_id(state):
    v = state.solver.Unconstrained('COPROC', 32, key=('hardware', 'coproc'))
    if hasattr(state,'coproc'):
        v = state.coproc.device_id
        #state.coproc.trace.append( ('mrc', state.regs.pc, v) )
    return v, []
