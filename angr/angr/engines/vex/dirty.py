...
def x86g_dirtyhelper_IN(state, portno, sz): #pylint:disable=unused-argument
    #print("ANGR IN CALLBACK", state.regs.ip, portno)
    portno = state.solver.eval(portno)
    val = state.solver.Unconstrained('IN_' + hex(portno), 32, key=('hardware', 'in'))
    if hasattr(state,'simdevices'):
        state.simdevices.r_count += 1
        state.simdevices.trace.append(('r',state.regs.eip,portno,sz,val))
    return val, [ ]

def x86g_dirtyhelper_OUT(state, portno, data, sz): #pylint:disable=unused-argument
    #print("ANGR OUT CALLBACK", state.regs.ip, portno)
    if hasattr(state,'simdevices'):
        state.simdevices.w_count += 1
        state.simdevices.trace.append(('w',state.regs.eip,portno,sz,data))
    return None, [ ]
...
def x86g_dirtyhelper_STI(state):
    print("ANGR STI Callback", hex(state.addr))
    if hasattr(state,"interrupts"):
        state.interrupts.do_sti()
    return None,[]

def x86g_dirtyhelper_CLI(state):
    print("ANGR CLI Callback", hex(state.addr))
    if hasattr(state,"interrupts"):
        state.interrupts.do_cli()
    return None,[]


def x86g_dirtyhelper_LLDT(state, selector):
    import IPython
    from .ccall import x86g_use_seg_selector
    #print("LLDT HAPPENING")
    selector = selector.zero_extend(16)
    offset = claripy.BVV(0,32)
    ldt_base = x86g_use_seg_selector(state,state.regs.ldt,state.regs.gdt,selector,offset)[0]
    gdt_base_val = state.solver.eval_one(state.regs.gdt[47:16])
    descriptor = state.memory.load(selector + gdt_base_val, 8, endness=state.arch.memory_endness)
    state.regs.ldt = descriptor[31:0].zero_extend(16)
    #IPython.embed()
    return None, [ ]
...
def x86g_dirtyhelper_LGDT_LIDT(state, addr, op):
    print("LGDT or LIDT happened: ", hex(state.addr), addr, op)
    print("before gdt ", state.regs.gdt)
    if not op.concrete:
        # resolved failed
        return None, [ ]
    ...
    if op._model_concrete.value == 2:
        ...
        if hasattr(state,'interrupts'):
            state.interrupts.do_lidt(base,limit)
    print("after gdt = " , state.regs.gdt)
    return None, [ ]
