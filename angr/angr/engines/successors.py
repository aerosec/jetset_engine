...
class SimSuccessors(object):
    ...
    def _manage_callstack(state):
        ...
        if state.history.jumpkind == 'Ijk_Call':
            ...
            #print("angr call to ", new_func_addr)
            ...
            try:
                if state.arch.call_pushes_ret:
                    if hasattr(state,'mmu'):
                        ret_addr = state.mem.int32_t[state.mmu.translate(state.regs.ss,state.regs._sp)].concrete
                        ret_addr = claripy.BVV(ret_addr,32)
                        ret_addr = state.solver.eval(state.mmu.translate(state.regs.cs, ret_addr))
                    else:
            ...
            #print("Pushing Angr Frame: ", hex(new_frame.call_site_addr), " => ", hex(new_frame.func_addr), "sp = ", hex(new_frame.stack_ptr),  "returns to: ", ("None" if not new_frame.ret_addr else hex(new_frame.ret_addr)))  
            ...
            while not state.arch.call_pushes_ret and \
                ...
                state._inspect('return', BP_BEFORE, function_address=state.callstack.top.func_addr)
                #print("Popping Angr Frame: ", hex(state.callstack.top.call_site_addr), " => ", hex(state.callstack.top.func_addr), "sp = ", hex(state.callstack.top.stack_ptr),  "returns to: ", ("None" if not state.callstack.top.ret_addr else hex(state.callstack.top.ret_addr)))
                frame = state.callstack.pop()
                #print("Popping Angr Frame: ", hex(frame.call_site_addr), " => ", hex(frame.func_addr), "sp = ", hex(frame.stack_ptr),  "returns to: ", hex(frame.ret_addr))
                #print("angr return: ", frame.ret_addr)
                state._inspect('return', BP_AFTER)
