...
def _load_native():
    ...
    try:
        ...
        h = type('Handle', (), {})()  # Just a container object
        h_c = ctypes.CDLL(angr_path)
        h_py = ctypes.PyDLL(angr_path)
        ...
        def _setup_prototype(handle, func, restype, *argtypes):
            ...
            setattr(h, func, getattr(handle, realname))
        ...
        _setup_prototype(h_c, 'alloc', state_t, uc_engine_t, ctypes.c_uint64)
        _setup_prototype(h_c, 'dealloc', None, state_t)
        _setup_prototype(h_c, 'hook', None, state_t)
        _setup_prototype(h_c, 'unhook', None, state_t)
        _setup_prototype(h_c, 'start', uc_err, state_t, ctypes.c_uint64, ctypes.c_uint64)
        _setup_prototype(h_c, 'stop', None, state_t, stop_t)
        _setup_prototype(h_c, 'sync', ctypes.POINTER(MEM_PATCH), state_t)
        _setup_prototype(h_c, 'bbl_addrs', ctypes.POINTER(ctypes.c_uint64), state_t)
        _setup_prototype(h_c, 'stack_pointers', ctypes.POINTER(ctypes.c_uint64), state_t)
        _setup_prototype(h_c, 'bbl_addr_count', ctypes.c_uint64, state_t)
        _setup_prototype(h_c, 'syscall_count', ctypes.c_uint64, state_t)
        _setup_prototype(h_c, 'destroy', None, ctypes.POINTER(MEM_PATCH))
        _setup_prototype(h_c, 'step', ctypes.c_uint64, state_t)
        _setup_prototype(h_c, 'stop_reason', stop_t, state_t)
        _setup_prototype(h_c, 'activate', None, state_t, ctypes.c_uint64, ctypes.c_uint64, ctypes.c_char_p)
        _setup_prototype(h_c, 'set_stops', None, state_t, ctypes.c_uint64, ctypes.POINTER(ctypes.c_uint64))
        _setup_prototype(h_c, 'cache_page', ctypes.c_bool, state_t, ctypes.c_uint64, ctypes.c_uint64, ctypes.c_char_p, ctypes.c_uint64)
        _setup_prototype(h_c, 'uncache_page', None, state_t, ctypes.c_uint64)
        _setup_prototype(h_c, 'enable_symbolic_reg_tracking', None, state_t, VexArch, _VexArchInfo)
        _setup_prototype(h_c, 'disable_symbolic_reg_tracking', None, state_t)
        _setup_prototype(h_c, 'symbolic_register_data', None, state_t, ctypes.c_uint64, ctypes.POINTER(ctypes.c_uint64))
        _setup_prototype(h_c, 'get_symbolic_registers', ctypes.c_uint64, state_t, ctypes.POINTER(ctypes.c_uint64))
        _setup_prototype(h_c, 'stopping_register', ctypes.c_uint64, state_t)
        _setup_prototype(h_c, 'stopping_memory', ctypes.c_uint64, state_t)
        _setup_prototype(h_c, 'is_interrupt_handled', ctypes.c_bool, state_t)
        _setup_prototype(h_c, 'set_transmit_sysno', None, state_t, ctypes.c_uint32, ctypes.c_uint64)
        _setup_prototype(h_c, 'process_transmit', ctypes.POINTER(TRANSMIT_RECORD), state_t, ctypes.c_uint32)
        _setup_prototype(h_c, 'set_tracking', None, state_t, ctypes.c_bool, ctypes.c_bool)
        _setup_prototype(h_c, 'executed_pages', ctypes.c_uint64, state_t)
        _setup_prototype(h_c, 'add_mmio_region', None, state_t, ctypes.c_uint64, ctypes.c_uint64)
        _setup_prototype(h_c, 'clear_mmio_regions', None, state_t)
        _setup_prototype(h_py, 'callstack_set', None, state_t, ctypes.py_object)
        _setup_prototype(h_py, 'callstack_get', ctypes.py_object, state_t)
...
def hack_eip(uc):
    eip = uc.state.solver.eval(uc.state.regs.eip)
    if hasattr(uc.state,'mmu'):
        seg_val = uc.state.solver.eval(uc.state.mmu.selector_base(uc.state.regs.cs))
        return eip - seg_val
    else:
        return eip
...
class Unicorn(SimStatePlugin):
    ...
    def __init__(
        ...
        cooldown_symbolic_registers=25,
        cooldown_symbolic_memory=25,
        cooldown_nonunicorn_blocks=25,
        #cooldown_symbolic_registers=10,
        #cooldown_symbolic_memory=10,
        #cooldown_nonunicorn_blocks=10,
        #cooldown_symbolic_registers=2,
        #cooldown_symbolic_memory=2,
        #cooldown_nonunicorn_blocks=2,
        cooldown_stop_point=2,
        max_steps=100000000,
    ):
        """
        ...
        self.countdown_nonunicorn_blocks = 50 #0
        self.countdown_symbolic_registers = 50 #0
        self.countdown_symbolic_memory = 50 #0
        self.countdown_stop_point = 50 #0
    ...
    def hook(self):
        ...
        _UC_NATIVE.clear_mmio_regions(self._uc_state)
        if hasattr(self.state, 'simmio'):
            for region in self.state.simmio.regions:
                _UC_NATIVE.add_mmio_region(self._uc_state, region.lower, region.upper)

        _UC_NATIVE.callstack_set(self._uc_state, ((
            frame.call_site_addr, frame.func_addr, frame.stack_ptr, frame.ret_addr)
            for frame in self.state.callstack))
        ...
        if arch == 'x86_64':
            ...
            self.uc.hook_add(unicorn.UC_HOOK_INTR, self._hook_intr_x86, None, 1, 0)
            self.uc.hook_add(unicorn.UC_HOOK_INSN, self._hook_in_x86, None, arg1=self._uc_const.UC_X86_INS_IN)
            self.uc.hook_add(unicorn.UC_HOOK_INSN, self._hook_out_x86, None, arg1=self._uc_const.UC_X86_INS_OUT)
            #self.uc.hook_add(unicorn.UC_HOOK_INSN, self._hook_cli_x86, None, arg1=self._uc_const.UC_X86_INS_CLI)
            #self.uc.hook_add(unicorn.UC_HOOK_INSN, self._hook_sti_x86, None, arg1=self._uc_const.UC_X86_INS_STI)
            #self.uc.hook_add(unicorn.UC_HOOK_INSN, self._hook_lidt_x86, None, arg1=self._uc_const.UC_X86_INS_LIDT)
            self.uc.hook_add(unicorn.UC_HOOK_INSN, self._hook_call_x86, None, arg1=self._uc_const.UC_X86_INS_CALL)
            self.uc.hook_add(unicorn.UC_HOOK_INSN, self._hook_ret_x86, None, arg1=self._uc_const.UC_X86_INS_RET)
        elif arch == 'mips':
            self.uc.hook_add(unicorn.UC_HOOK_INTR, self._hook_intr_mips, None, 1, 0)
        elif arch == 'mipsel':
            self.uc.hook_add(unicorn.UC_HOOK_INTR, self._hook_intr_mips, None, 1, 0)
        elif arch == 'arm':
            self.uc.hook_add(unicorn.unicorn_const.UC_HOOK_MEM_WRITE, self._hook_simram)
            self.uc.hook_add(unicorn.UC_HOOK_INTR, self._hook_intr_arm, None, 1, 0)
        elif arch == 'm68k':
            pass
    ...
    def _hook_simram(self, uc, access, address, size, value, user_data):
        if not hasattr(self.state, 'simram'):
            return

        pc = self.uc.reg_read(unicorn.arm_const.UC_ARM_REG_PC)

        for region in self.state.simram.regions:
            if address >= region.lower and address <= region.upper:
                if access == unicorn.unicorn_const.UC_MEM_WRITE:
                    print(">>>>>>>>>>>>> Memory is being WRITE at 0x%x, data size = %u, data value = 0x%x pc = 0x%x" %(address, size, value, pc))

    def _hook_in_x86(self, uc, port, size, data):
        print("IN CALLBACK: port = ", hex(port), " size = ", size)
        _UC_NATIVE.stop(self._uc_state, STOP.STOP_NORMAL)
        return 0

    def _hook_cli_x86(self, uc):
        print("CLI CALLBACK")
        return 0

    def _hook_sti_x86(self, uc):
        print("STI CALLBACK")
        return 0

    def _hook_lidt_x86(self, uc):
        print("LIDT CALLBACK")
        return 0

    def _hook_out_x86(self, uc, port, size, v, data):
        #print("OUT CALLBACK: port = ", hex(port), " size = ", size,"value = ", hex(v) )
        if hasattr(self.state,"simdevices"):
            self.state.simdevices.w_count += 1
            self.state.simdevices.trace.append(('w',self.state.regs.eip,port,size,v))
        return 0

    def _hook_ret_x86(self, uc, target, user_data):
        new_frame = self.state.callstack.pop()
        #print("Popping Unicorn Frame: ", hex(self.state.callstack.top.call_site_addr), " => ", hex(self.state.callstack.func_addr), "sp = ", hex(self.state.callstack.top.stack_ptr),  "returns to: ", self.state.callstack.top.ret_addr)
        #print(">>>ret popped", hex(new_frame.call_site_addr))

    def _hook_call_x86(self, uc, target, ret_addr, user_data):
        eip = self.uc.reg_read(unicorn.x86_const.UC_X86_REG_EIP)
        esp =  self.uc.reg_read(unicorn.x86_const.UC_X86_REG_ESP)
        # TODO: in x86 eip is untranslated, i.e. not offset by code segment
        if hasattr(self.state,'mmu'):
            cs = self.uc.reg_read(unicorn.x86_const.UC_X86_REG_CS)
            target_bv = claripy.BVV(target, 32)
            cs_bv = claripy.BVV(cs, 16)
            target = self.state.solver.eval(self.state.mmu.translate(cs_bv, target_bv))

            ret_addr_bv = claripy.BVV(ret_addr, 32)
            ret_addr = self.state.solver.eval(self.state.mmu.translate(cs_bv, ret_addr_bv))

            #ss = self.uc.reg_read(unicorn.x86_const.UC_X86_REG_SS)
            #ss_bv = claripy.BVV(cs, 16)
            #esp_bv = claripy.BVV(target, 32)
            #ret_on_stack = self.state.solver.eval(self.state.mmu.translate(ss_bv, esp_bv))
            #ret_addr = self.uc.mem_read(ret_on_stack, 4)
            #import IPython
            #import sys
            #IPython.embed()
            #sys.exit()
        ...
        new_frame = CallStack(func_addr=target, call_site_addr = eip, jumpkind='Ijk_Call', stack_ptr = esp, ret_addr = ret_addr)
        #print("Pushing Unicorn Frame: ", hex(new_frame.call_site_addr), " => ", hex(new_frame.func_addr), "sp = ", hex(new_frame.stack_ptr),  "returns to: ", hex(new_frame.ret_addr))
        self.state.callstack.push(new_frame)
    ...
    def _hook_intr_arm(self, uc, intno, user_data):
        self.trap_ip = self.uc.reg_read(unicorn.arm_const.UC_ARM_REG_PC)
        l.warning('unhandled interrupt %d', intno)
        _UC_NATIVE.stop(self._uc_state, STOP.STOP_ERROR)
    ...
    def _hook_intr_x86(self, uc, intno, user_data):
        ...
        if intno == 0:
            ...
            l.warning('unhandled interrupt %d @ %s', intno,  hex(self.trap_ip))
            esp = self.uc.reg_read(unicorn.x86_const.UC_X86_REG_ESP)
            print("esp = ", hex(esp))
            #IPython.embed()
            _UC_NATIVE.stop(self._uc_state, STOP.STOP_ERROR)
    ...
    def _hook_mem_unmapped_core(self, uc, access, start, length, best_effort_read=True):
        ...
        # do the mapping
        l.info('mmap [%#x, %#x], %d%s (because )', start, start + length - 1, perm, ' (symbolic)' if taint else '')
        if not taint and not perm & 2:
            ...
            #print("preparing to mem_map to ", hex(start))
            uc.mem_map(start, length, perm)
            #print("preparing to mem_write to ", hex(start))
            uc.mem_write(start, bytes(data))
            #print("completed mem_write to ", hex(start))
            self._mapped += 1
    ...
    def setup(self):
        ...
        if self.state.arch.qemu_name == 'i386':
            _UC_NATIVE.activate(self._uc_state, G_GDT_ADDR, G_GDT_LIMIT, None)
            if G_LDT_LIMIT > 0:
                _UC_NATIVE.activate(self._uc_state, G_LDT_ADDR, G_LDT_LIMIT, None)




    def start(self, step=None):
        print("Entering Unicorn @ ", self.state.ip)
        #print(self.state.callstack)
        #for frame in self.state.callstack:
        #    print(hex(frame.ret_addr))
        self.jumpkind = 'Ijk_Boring'
    ...
    def finish(self):
        ...
        if self.stop_reason == STOP.STOP_SYMBOLIC_REG:
            ...
            self._report_symbolic_blocker(self.state.registers.load(stopping_register, 1, inspect=False), 'reg')
        elif self.stop_reason == STOP.STOP_SYMBOLIC_MEM:
            stopping_memory = _UC_NATIVE.stopping_memory(self._uc_state)
            #print("stopping due to symbolic memory", stopping_memory, self.state.memory.load(stopping_memory, 1, inspect=False))
            self._report_symbolic_blocker(self.state.memory.load(stopping_memory, 1, inspect=False), 'mem')

        #print("number of steps = ", self.steps)
        if self.stop_reason == STOP.STOP_NOSTART and self.steps > 0:
            print("============ Something has gone horribly wrong with unicorn =========")
            raise SimUnicornError("Got STOP_NOSTART but a positive number of steps. This indicates a serious unicorn bug.")

        addr = self.state.solver.eval(self.state.ip)
        #addr = self.state.solver.eval(translate_eip(self,self.state.regs.eip))
        l.info('finished emulation at %#x after %d steps: %s', addr, self.steps, STOP.name_stop(self.stop_reason))
        print('finished emulation at {} after {} steps: {}'.format(hex(addr), self.steps, STOP.name_stop(self.stop_reason)))
        #print(self.state.callstack)
        #for frame in self.state.callstack:
        #    print(hex(frame.ret_addr))
        ...
        while bool(p_update):
            ...
            l.info('...changed memory: [%#x, %#x]', address, address + length)
            if update.has_value:
                s = update.value
            else:
                s = bytes(self.uc.mem_read(address, int(length)))
            self.state.memory.store(address, s, length)
        ...
        uc_callstack = _UC_NATIVE.callstack_get(self._uc_state)
        for n_frame_same, (angr_frame, uc_frame) in enumerate(
                zip(reversed(self.state.callstack), reversed(uc_callstack))):
            call_site_addr, func_addr, stack_ptr, ret_addr = uc_frame
            if (angr_frame.call_site_addr != call_site_addr or
                    angr_frame.func_addr != func_addr or
                    angr_frame.stack_ptr != stack_ptr or
                    angr_frame.ret_addr != ret_addr):
                n_frame_same -= 1
                break
        n_frame_same += 1
        assert n_frame_same

        for i in range(len(self.state.callstack) - n_frame_same):
            self.state.callstack.pop()
        for uc_frame in reversed(uc_callstack[:-n_frame_same]):
            call_site_addr, func_addr, stack_ptr, ret_addr = uc_frame
            new_frame = CallStack(func_addr=func_addr, call_site_addr=call_site_addr, jumpkind='Ijk_Call', stack_ptr=stack_ptr, ret_addr=ret_addr)
            self.state.callstack.push(new_frame)

        if self.stop_reason in (STOP.STOP_NORMAL, STOP.STOP_SYSCALL):
            self.countdown_nonunicorn_blocks = 2
        ...
        elif self.stop_reason == STOP.STOP_EXECNONE:
            self.countdown_unicorn_blocks = 2
        else:
            self.countdown_nonunicorn_blocks = self.cooldown_nonunicorn_blocks
    ...
    def set_regs(self):
        ...
        if self.state.arch.qemu_name == 'x86_64':
            ...
            #print("I should be setting LDT up here")
    ...
    def copy_page(self, start, length):
        #print("copying page!", hex(start), length)
        items = self.state.memory.mem.load_objects(start, length)
        #for mo_addr,mo in items:
        #    print(hex(mo_addr), hex(mo.base), mo.length)
        data = bytearray(length)
        taint = []
        uc = self.uc
    ...
    def setup_gdt(self):
        #1. Extract gdt_limit_base
        gdt_limit = self.state.solver.eval_one(self.state.regs.gdt[15:0])
        gdt_base = self.state.solver.eval_one(self.state.regs.gdt[47:16])


        ldt_limit = self.state.solver.eval_one(self.state.regs.ldt[15:0])
        ldt_base = self.state.solver.eval_one(self.state.regs.ldt[47:16])
        #print("ldt_base = ", ldt_base, " ldt_limit = ", ldt_limit)
        if ldt_limit != 0:
            LDT_ADDR = ldt_base
            LDT_ADDR_ALIGNED = LDT_ADDR & (0xffffffffffffff000)
            LDT_LIMIT = (ldt_limit + 0xfff) & 0xffffffffffffff000


        GDT_ADDR = gdt_base
        GDT_ADDR_ALIGNED = GDT_ADDR & (0xffffffffffffff000)
        GDT_LIMIT = (gdt_limit + 0xfff) & 0xffffffffffffff000

        uc = self.uc
        cs = self.state.solver.eval_one(self.state.regs.cs)
        ds = self.state.solver.eval_one(self.state.regs.ds)
        es = self.state.solver.eval_one(self.state.regs.es)
        fs = self.state.solver.eval_one(self.state.regs.fs)
        gs = self.state.solver.eval_one(self.state.regs.gs)
        ss = self.state.solver.eval_one(self.state.regs.ss)

        global G_GDT_ADDR
        global G_GDT_LIMIT
        G_GDT_ADDR  = GDT_ADDR_ALIGNED
        G_GDT_LIMIT = GDT_LIMIT


        #2. Map the gdt
        #uc.mem_map(GDT_ADDR_ALIGNED, GDT_LIMIT)
        self.copy_page(GDT_ADDR_ALIGNED, GDT_LIMIT)
        #3. Map the ldt
        if ldt_limit != 0:
            self.copy_page(LDT_ADDR_ALIGNED, LDT_LIMIT)
            #uc.mem_map(LDT_ADDR_ALIGNED, LDT_LIMIT)


        #Copy GDT Values to global context so unicorn knows they are there
        #global G_GDT_ADDR
        #global G_GDT_LIMIT
        #G_GDT_ADDR  = GDT_ADDR
        #G_GDT_LIMIT = GDT_LIMIT
        ...
        global G_LDT_LIMIT
        if ldt_limit != 0:
            G_LDT_ADDR = LDT_ADDR_ALIGNED
            G_LDT_LIMIT = LDT_LIMIT

        #Copy GDT to unicorn's memory
        #for addr in range(GDT_ADDR, GDT_ADDR + gdt_limit, 8):
        #    self.copy_entry(addr)
        #self.copy_page(GDT_ADDR_ALIGNED, GDT_LIMIT)

        #print("setup gdt thing")
        #IPython.embed()


        #if ldt_limit != 0:
        #    self.copy_page(LDT_ADDR_ALIGNED, LDT_LIMIT)
        #    #for addr in range(LDT_ADDR, LDT_ADDR + ldt_limit, 8):
        #    #    self.copy_entry(addr)

        #Copy relevant registers
        uc.reg_write(self._uc_const.UC_X86_REG_GDTR, (0, gdt_base, gdt_limit, 0x0))
        if ldt_limit != 0:
            uc.reg_write(self._uc_const.UC_X86_REG_LDTR, (0, ldt_base, ldt_limit, 0x0))

        uc.reg_write(self._uc_const.UC_X86_REG_CS, cs)
        uc.reg_write(self._uc_const.UC_X86_REG_DS, ds)
        uc.reg_write(self._uc_const.UC_X86_REG_ES, es)
        uc.reg_write(self._uc_const.UC_X86_REG_SS, ss)
        uc.reg_write(self._uc_const.UC_X86_REG_FS, fs)
        uc.reg_write(self._uc_const.UC_X86_REG_GS, gs)

        #_UC_NATIVE.activate(self._uc_state, G_GDT_ADDR, G_GDT_LIMIT, None)
        #if G_LDT_LIMIT > 0:
        #    _UC_NATIVE.activate(self._uc_state, G_LDT_ADDR, G_LDT_LIMIT, None)

    def copy_entry(self, addr):
        entry = self.state.memory.load(addr, 8, endness=self.state.arch.memory_endness)
        entry = self.state.solver.eval_one(entry)
        entry = struct.pack('<Q', entry)
        self.uc.mem_write(addr, entry)
    ...
    def get_regs(self):
        ...
        if self.state.arch.qemu_name == 'i386':
            #fix up gdt
            v = self.uc.reg_read(self._uc_const.UC_X86_REG_GDTR)
            gdt_base = claripy.BVV(v[1],32)
            gdt_limit = claripy.BVV(v[2],16)
            gdt = claripy.Concat(gdt_base, gdt_limit)
            self.state.regs.gdt = gdt
            #print("recovered gdt = ", gdt)
            v = self.uc.reg_read(self._uc_const.UC_X86_REG_LDTR)
            #print("unicorn ldt: ", v)
            ldt_base = claripy.BVV(v[1],32)
            ldt_limit = claripy.BVV(v[2],16)
            ldt = claripy.Concat(ldt_base, ldt_limit)
            self.state.regs.ldt = ldt
            #print("recovered ldt = ", ldt)

        #IPython.embed()
        # now we sync registers out of unicorn
        for r, c in self._uc_regs.items():
            #if r in self.reg_blacklist:
            #    print("reg blacklist: ", r)
            #    continue
            if r == 'eip':
                v = self.uc.reg_read(c)
                #v = hack_eip(self)
                #print('retrieved_eip = ', hex(self.uc.reg_read(c)), "unhacked_eip = ", v )
            else:
                v = self.uc.reg_read(c)
            l.debug('getting $%s = %#x', r, v)
            setattr(self.state.regs, r, v)

        #print("Exiting cs: ", self.state.regs.cs)
        #print("Exiting ds: ", self.state.regs.ds)
        #print("Exiting es: ", self.state.regs.es)
        #print("Exiting fs: ", self.state.regs.fs)
        #print("Exiting gs: ", self.state.regs.gs)
        #print("Exiting ss: ", self.state.regs.ss)
