...
class SimSymbolicMemory(SimMemory): #pylint:disable=abstract-method
    ...
    def _resolve_size_range(self, size):
        if not self.state.solver.symbolic(size):
            ...
            if i > (self._maximum_concrete_size + 1):
                raise SimMemoryLimitError("Concrete size %d outside of allowable limits max size = %d" % (i,self._maximum_concrete_size))
            return i, i
    ...
    def _apply_concretization_strategies(self, addr, strategies, action):
        ...
        raise SimMemoryAddressError(
            "Unable to concretize address %r for %s with the provided strategies." % (addr, action)
        )
    ...
    def normalize_address(self, addr, is_write=False):
        print("Normalizing Address! ", addr)
        ...
        if (all_missing and
            ...
            if self.category == 'reg':
                ...
                l.warning("Register %s has an unspecified value; "
                          "Generating an unconstrained value of %d bytes. @ %s", reg_str, num_bytes, str(self.state.regs.pc))
            elif self.category == 'mem':
                l.warning("Memory address %#x has an unspecified value; "
                          "Generating an unconstrained value of %d bytes. @ %s", addr, num_bytes, str(self.state.regs.pc))
