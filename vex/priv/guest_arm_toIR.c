   ...
   */
   {
     ...
     if (gate) {
        ...
        if (isLoad) {
           ...
           loadGuarded(data, cvt[szBlg2], ea, mkU32(0)/*alt*/, condT);
           if (isT) {
   ...
   */
   {
     ...
     if (gate) {
        ...
        if (isLoad) {
           ...
           stmt( IRStmt_LLSC(guest_memory_endness, res, ea, NULL/*this is a load*/) );
...
#          define GET_IREG(_nnz) (isT ? getIRegT(_nnz) : getIRegA(_nnz))
           assign(data,
                  ty == Ity_I64
                     ? binop(Iop_32HLto64,
                        guest_endness == VexEndnessLE ? GET_IREG(tt2) : GET_IREG(tt),
                        guest_endness == VexEndnessLE ? GET_IREG(tt) : GET_IREG(tt2))
                     : narrow == Iop_INVALID
                        ? GET_IREG(tt)
                         ...
                         UInt regList )
{
   ...
   IRTemp jk = Ijk_Boring;
    ...
    //if (bL == 1)
    //	   printf("LDM being handled in native!\n");
...
#  define INSN_COND          SLICE_UInt(insn, 31, 28)
   ...
   */
   ...
   /* --------------------- Msr etc --------------------- */
    /* MRC, MCR*/
    /*
    if (INSN(27,24) == BITS4(1,1,1,0) && INSN(4) == BITS1(1)){
      if INSN(20) == BITS1(0)//mcr
      {
        
      }
      else if INSN(20) == BITS1(1)//mrc
      {
   ...
   if (0x01800F90 == (insn & 0x0F900FF0)) {
      ...
      if (valid) {
         ...
         assign(data,
                ty == Ity_I64
                   ? binop(Iop_32HLto64,
                      guest_endness == VexEndnessLE ? getIRegA(rT+1) : getIRegA(rT+0),
                      guest_endness == VexEndnessLE ? getIRegA(rT+0) : getIRegA(rT+1))
                   : narrow == Iop_INVALID
                      ? getIRegA(rT)
